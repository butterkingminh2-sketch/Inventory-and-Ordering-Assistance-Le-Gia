...
const badge = {
  sufficient: { vi: 'Đủ',      en: 'Sufficient', classes: 'bg-secondary-container text-on-secondary-container' },
  low:        { vi: 'Sắp hết', en: 'Low stock',   classes: 'bg-tertiary-container text-on-tertiary-container' },
  out:        { vi: 'Hết',     en: 'Out of stock', classes: 'bg-error text-on-error' },
}
...
export function IngredientCard({ item, onAdjust, onSetQuantity }: Props) {
  ...
  const itemName = pickName(item, language)

  return (
    <article className={cardWrapper[status]}>
      <div className="flex items-start justify-between gap-2 mb-3">
        <div className="flex-1 min-w-0">
          <p className="text-headline-md font-bold text-on-surface truncate">{itemName}</p>
        </div>
        <span className={`shrink-0 text-status-badge font-black uppercase tracking-wider px-2 py-0.5 rounded-full ${badge[status].classes}`}>
          <BilingualText vi={badge[status].vi} en={badge[status].en} />
        </span>
      </div>
        ...
        <div>
          {editing ? (
            ...
            <button
              ...
              aria-label={`Sửa số lượng ${itemName}`}
